...
exports.googleAdsService = exports.GoogleAdsKeywordService = void 0;
...
class GoogleAdsKeywordService {
     ...
     */
    async generateKeywordIdeas(seedKeywords, domain, location) {
        try {
            ...
            try {
                ...
                console.warn('Google Ads API failed, falling back to AI generation:', googleAdsError instanceof Error ? googleAdsError.message : String(googleAdsError));
            }
     ...
     */
    async generateKeywordsWithAI(seedKeywords, domain, location) {
        if (!this.openai) {
...
[
  {
     ...
     */
    async analyzeBusinessDomain(domain, seedKeywords) {
        if (!this.openai) {
...
Provide analysis in JSON format:
{
  ...
  "keywordOpportunities": "specific keyword themes to target"
     ...
     */
    getBasicBusinessAnalysis(domain) {
        const cleanDomain = domain.replace(/^https?:\/\//, '').replace(/^www\./, '').split('.')[0];
     ...
     */
    extractDetailedDomainContext(domain) {
        const cleanDomain = domain.replace(/^https?:\/\//, '').replace(/^www\./, '').split('.')[0];
     ...
     */
    getDetailedLocationContext(location) {
        const locationMap = {
     ...
     */
    parseEnhancedAIResponse(responseText, seedKeywords, domain, location) {
        const keywords = [];
     ...
     */
    estimateSearchVolume(term, domain, location) {
        const words = term.split(' ');
        const wordCount = words.length;
        // Base volume based on word count
        let baseVolume = 50;
        if (wordCount === 1)
            baseVolume = 80; // Short-tail
        else if (wordCount === 2)
            baseVolume = 60; // Medium-tail
        else if (wordCount === 3)
            baseVolume = 40; // Long-tail
        else
            baseVolume = 20; // Very long-tail
     ...
     */
    estimateCompetition(term, domain) {
        const words = term.split(' ');
        const wordCount = words.length;
        // Base competition based on word count
        if (wordCount === 1)
            return 'High'; // Short-tail = high competition
        else if (wordCount === 2)
            return 'Medium'; // Medium-tail = medium competition
        else
            return 'Low'; // Long-tail = low competition
        // Additional factors could be considered here
    }
    /**
     * Estimate CPC based on keyword characteristics
     */
    estimateCPC(term, domain) {
        const words = term.split(' ');
        const wordCount = words.length;
        // Base CPC based on word count
        let baseCPC = 1.50;
        if (wordCount === 1)
            baseCPC = 3.00; // Short-tail = higher CPC
        else if (wordCount === 2)
            baseCPC = 2.00; // Medium-tail = medium CPC
        else
            baseCPC = 1.00; // Long-tail = lower CPC
        // Adjust for commercial intent
        const commercialTerms = ['buy', 'price', 'cost', 'cheap', 'best', 'review', 'compare'];
        const hasCommercialIntent = commercialTerms.some(term => term.includes(term));
        if (hasCommercialIntent)
     ...
     */
    generateComprehensiveFallbackKeywords(seedKeywords, domain, location) {
        const keywords = [];
        const domainWords = domain.replace(/^https?:\/\//, '').replace(/^www\./, '').split('.')[0].split(/[-_]/);
     ...
     */
    generateKeywordVariations(seed, domainWords, location) {
        const variations = [];
     ...
     * Generate domain-specific keywords
     */
    generateDomainSpecificKeywords(domainWords, location) {
        const keywords = [];
     ...
     * Generate location-specific keywords
     */
    generateLocationSpecificKeywords(seedKeywords, location) {
        const keywords = [];
     ...
     */
    async getKeywordIdeasFromPlanner(customer, seedKeyword, location) {
        try {
            ...
            const request = {
                ...
                keyword_plan_network: google_ads_api_1.enums.KeywordPlanNetwork.GOOGLE_SEARCH_AND_PARTNERS,
     ...
     */
    async getKeywordDataFromSearchVolume(customer, seedKeyword, location) {
        try {
            // Build geo target constants
     ...
     */
    filterRelevantKeywords(keywords, seedKeywords, domain) {
        ...
        return keywords
            ...
            return keywordWords.some(word => allSeedWords.some(seed => word.includes(seed.toLowerCase()) || seed.toLowerCase().includes(word)));
        })
            .sort((a, b) => b.volume - a.volume)
            .slice(0, 30);
    }
     ...
     */
    extractDomainWords(domain) {
        const cleanDomain = domain.replace(/^https?:\/\//, '').replace(/^www\./, '').split('.')[0];
        return cleanDomain.split(/[-_]/).filter(word => word.length > 2);
    }
}
exports.GoogleAdsKeywordService = GoogleAdsKeywordService;
// Helper function to generate seed keywords from domain context
function generateSeedKeywords(domain, context) {
    const seedKeywords = [];
...
exports.googleAdsService = createGoogleAdsService();
