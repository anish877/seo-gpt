...
exports.IntentPhraseService = void 0;
...
if (!SERP_API_KEY)
    throw new Error('SERP_API_KEY not set in environment variables');
const openai = new openai_1.default({ apiKey: OPENAI_API_KEY });
const prisma = new prisma_1.PrismaClient();
// SERP API helper functions
const searchSerpApi = async (query, engine) => {
...
const extractRedditData = (serpData) => {
    if (!serpData || !serpData.organic_results)
        return [];
    return serpData.organic_results
        .filter((result) => result.link && result.link.includes('reddit.com'))
        .map((result) => ({
        title: result.title || '',
...
const extractQuoraData = (serpData) => {
    if (!serpData || !serpData.organic_results)
        return [];
    return serpData.organic_results
        .filter((result) => result.link && result.link.includes('quora.com'))
        .map((result) => ({
        title: result.title || '',
        content: result.snippet || '',
        url: result.link || '',
...
class IntentPhraseService {
    constructor(domainId, keywords, domain, onProgress, onPhraseGenerated) {
...
Provide a comprehensive analysis that will be used to inform community research, content strategy, and search phrase generation.`;
            ...
            if (response) {
                ...
                await prisma.semanticAnalysis.create({
                    data: {
                        ...
                        keyThemes: semanticData.themes?.primary?.map((t) => t.theme) || [],
    ...
    async communityDataMining() {
        ...
        for (let i = 0; i < this.keywords.length; i++) {
            ...
            try {
                ...
                let redditData = null;
                try {
                    redditData = await this.mineRedditFromTargetCommunities(keyword.term, searchQueries.reddit, targetSubreddits, semanticContext);
                ...
                let quoraData = null;
                try {
                    quoraData = await this.mineQuoraFromTargetSpaces(keyword.term, searchQueries.quora, targetQuoraSpaces, semanticContext);
    ...
    generateContextualSearchQueries(keyword, semanticContext) {
        ...
        const redditQueries = [
            ...
            ...Object.values(faqs).flat().slice(0, 3).map((faq) => `${keyword} ${faq}`),
            // Use target audience context
            `${keyword} for ${targetAudience.demographics?.profession || 'business professionals'}`,
            // Use brand themes
            ...themes.slice(0, 2).map((theme) => `${keyword} ${theme.theme}`),
            // Pain point focused (from target audience analysis)
            ...(targetAudience.personas?.[0]?.painPoints || []).slice(0, 2).map((pain) => `${keyword} ${pain}`),
            // Generic patterns
    ...
    async mineRedditFromTargetCommunities(keyword, queries, targetSubreddits, semanticContext) {
        const allRedditData = [];
        for (const query of queries) {
            try {
                ...
                const filteredResults = redditResults.filter((result) => targetSubreddits.some((sub) => result.subreddit && result.subreddit.toLowerCase().includes(sub.name.replace('r/', '').toLowerCase())));
    ...
    async mineQuoraFromTargetSpaces(keyword, queries, targetSpaces, semanticContext) {
        const allQuoraData = [];
        for (const query of queries) {
            try {
    ...
    extractRealFAQsFromCommunityData(redditData, quoraData, expectedFAQs) {
        const extractedFAQs = {
            productService: [],
            process: [],
            comparison: [],
    ...
    calculateRelevanceToSemantic(post, semanticContext) {
        ...
        painPoints.forEach((pain) => {
            if (content.includes(pain.toLowerCase()))
                relevanceScore += 15;
        });
        // Check theme alignment
        const themes = semanticContext.themes?.primary || [];
        themes.forEach((theme) => {
            if (content.includes(theme.theme.toLowerCase()))
                relevanceScore += 10;
        });
        return Math.min(100, relevanceScore);
    }
    analyzeSubredditData(redditData) {
        const subredditInsights = new Map();
        redditData.forEach((result) => {
            if (result.subreddit) {
        ...
        return Array.from(subredditInsights.entries()).map(([subreddit, data]) => ({
            subreddit,
    ...
    analyzeQuoraTopicData(quoraData) {
        ...
        return Array.from(topicInsights.entries()).map(([topic, data]) => ({
            topic,
    ...
    async intentClassificationEngine() {
        this.onProgress({
    ...
    async relevanceScoreCalculation(phrases) {
        this.onProgress({
    ...
    async mineRedditData(keyword, semanticContext) {
...
` : '';
    ...
    async mineQuoraData(keyword, semanticContext) {
        const contextInfo = semanticContext ? `
Brand Voice: ${JSON.stringify(semanticContext.brandVoice || {})}
Target Audience: ${JSON.stringify(semanticContext.targetAudience || {})}
Themes: ${JSON.stringify(semanticContext.themes || [])}
...
` : '';
    ...
    async analyzeSearchPatterns(keyword, semanticContext) {
        const contextInfo = semanticContext ? `
Brand Voice: ${JSON.stringify(semanticContext.brandVoice || {})}
Target Audience: ${JSON.stringify(semanticContext.targetAudience || {})}
Themes: ${JSON.stringify(semanticContext.themes || [])}
...
` : '';
    ...
    async classifyIntent(keyword, semanticContext) {
        const contextInfo = semanticContext ? `
Brand Voice: ${JSON.stringify(semanticContext.brandVoice || {})}
Target Audience: ${JSON.stringify(semanticContext.targetAudience || {})}
Themes: ${JSON.stringify(semanticContext.themes || [])}
...
` : '';
    ...
    async generatePhrasesForKeyword(keyword, semanticContext) {
        const contextInfo = semanticContext ? `
Brand Voice: ${JSON.stringify(semanticContext.brandVoice || {})}
Target Audience: ${JSON.stringify(semanticContext.targetAudience || {})}
Themes: ${JSON.stringify(semanticContext.themes || [])}
...
- Phrases should align with brand voice and target audience insights`;
    ...
    async calculateRelevanceScore(phrase) {
...
Return ONLY the numeric score between 60-95.`;
    ...
    async saveCommunityMiningResult(keywordId, platform, data) {
    ...
    async saveSearchPatternResult(keywordId, data) {
    ...
    async saveIntentClassificationResult(keywordId, data) {
    ...
    async saveRelevanceScoreResult(phrase, score) {
            ...
            if (competitorAnalysis.length > 0) {
                await prisma.competitorAnalysis.createMany({
                    data: competitorAnalysis.map(analysis => ({
                        ...
                        marketInsights: { insights: analysis.competitors.map((c) => c.overview) },
                        strategicRecommendations: { recommendations: analysis.competitors.map((c) => c.differentiators) },
                        competitiveAnalysis: { analysis: analysis.competitors },
                        competitorList: analysis.competitors.map((c) => c.name).join(', ')
                    }))
                });
            }
            this.onProgress({
    ...
    async extractCompetitorsFromCommunityData(insightData, semanticContext) {
        ...
        if (insightData.realInsights?.reddit?.posts) {
            insightData.realInsights.reddit.posts.forEach((post) => {
                const content = (post.title + ' ' + post.content).toLowerCase();
        ...
        if (insightData.realInsights?.quora?.questions) {
            insightData.realInsights.quora.questions.forEach((question) => {
        ...
        if (insightData.extractedFAQs) {
            Object.values(insightData.extractedFAQs).flat().forEach((faq) => {
    ...
    extractCompetitorMentions(content, domainName) {
        const competitors = [];
    ...
    async researchCompetitors(competitors, semanticContext) {
...
`;
    ...
    calculateCompetitorConfidence(competitor, semanticContext) {
...
exports.IntentPhraseService = IntentPhraseService;
exports.default = IntentPhraseService;
