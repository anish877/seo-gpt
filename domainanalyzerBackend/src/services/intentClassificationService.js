...
async function classifyIntent(keywords, domain, location) {
    try {
...
- Focus on conversion optimization and user experience improvements`;
        ...
        const transactionalKeywords = keywords
            .filter((k) => k.intent === 'Transactional')
            .map((k) => ({
            term: k.term,
        ...
        const userJourneyMapping = {
            awareness: [...informationalKeywords, ...navigationalKeywords].slice(0, 5).map((k) => k.term),
            consideration: [...commercialKeywords].slice(0, 5).map((k) => k.term),
            decision: [...commercialKeywords].slice(5, 10).map((k) => k.term),
            action: [...transactionalKeywords].slice(0, 5).map((k) => k.term)
        };
        return {
