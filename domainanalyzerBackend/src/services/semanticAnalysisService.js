...
exports.analyzeSemanticContent = analyzeSemanticContent;
const openai_1 = __importDefault(require("openai"));
const OPENAI_API_KEY = process.env.OPENAI_API_KEY;
if (!OPENAI_API_KEY)
    throw new Error('OPENAI_API_KEY not set in environment variables');
const openai = new openai_1.default({ apiKey: OPENAI_API_KEY });
async function analyzeSemanticContent(content, domain, location) {
    try {
        const prompt = `You are an expert content strategist and semantic analysis specialist. Conduct a comprehensive semantic analysis that builds upon the domain context analysis to provide deeper content insights.
