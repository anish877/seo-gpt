...
exports.generateCampaignTopics = generateCampaignTopics;
exports.generatePillarPageSuggestion = generatePillarPageSuggestion;
exports.generateSubPagesSuggestion = generateSubPagesSuggestion;
exports.generateKeywordsSuggestion = generateKeywordsSuggestion;
...
const callOpenAiJson = async (prompt, fallback) => {
    if (!openai) {
        return fallback();
    }
    try {
...
const buildKeyword = (keyword, seed) => {
    if (!keyword && seed) {
        return {
            term: seed,
...
const buildPage = (page, seed) => {
    ...
    return {
        ...
        keywords: Array.isArray(page.keywords)
            ? page.keywords.map((kw) => buildKeyword(kw))
            : seed
                ? [buildKeyword(undefined, `${seed} outline`)]
                : []
    };
};
const buildTopic = (topic, fallbackSeed) => {
    ...
    return {
        ...
        subPages: Array.isArray(topic?.subPages)
            ? topic.subPages.map((page, index) => buildPage(page, `${seed} sub ${index + 1}`))
            : [],
        keywords: Array.isArray(topic?.keywords)
            ? topic.keywords.map((kw) => buildKeyword(kw))
            : []
    };
};
async function generateCampaignTopics(context) {
...
Only return JSON.`;
    const fallback = () => {
        return Array.from({ length: count }).map((_, index) => buildTopic({
            title: focus
                ? `${focus} Topic ${index + 1}`
    ...
    const aiResponse = await callOpenAiJson(prompt, fallback);
    const topics = Array.isArray(aiResponse?.topics) && aiResponse.topics.length > 0
        ? aiResponse.topics
        : fallback();
    return topics.map((topic, index) => buildTopic(topic, `Topic ${index + 1}`));
}
async function generatePillarPageSuggestion(context) {
...
async function generateSubPagesSuggestion(context) {
...
Only return JSON.`;
    const fallback = () => Array.from({ length: count }).map((_, index) => buildPage({
    ...
    const subPages = Array.isArray(aiResponse?.subPages) && aiResponse.subPages.length > 0
        ? aiResponse.subPages
        : fallback();
    return subPages.map((page, index) => buildPage(page, `${topicTitle} sub ${index + 1}`));
}
async function generateKeywordsSuggestion(context) {
...
Only return JSON.`;
    const fallback = () => Array.from({ length: count }).map((_, index) => buildKeyword({
    ...
    const keywordList = Array.isArray(aiResponse?.keywords) && aiResponse.keywords.length > 0
        ? aiResponse.keywords
        : fallback();
    return keywordList.map((kw, index) => buildKeyword(kw, `${scope} keyword ${index + 1}`));
}
