...
const campaignAiService_1 = require("../services/campaignAiService");
...
const serializeTopic = (topic) => {
    const pillar = topic.pages.find((page) => page.pageType === prisma_1.CampaignPageType.PILLAR) || null;
    const subPages = topic.pages
        .filter((page) => page.pageType === prisma_1.CampaignPageType.SUBPAGE)
...
const serializeStructure = (campaign) => ({
    topics: campaign.topics
        .slice()
        .sort((a, b) => a.order - b.order)
        .map(serializeTopic)
});
const fetchCampaignStructure = async (campaignId, userId) => {
...
const respondWithStructure = async (res, campaignId, userId, status = 200) => {
...
const ensureCampaignOwnership = async (campaignId, userId) => {
    return prisma.campaign.findFirst({
        where: {
            id: campaignId,
            domain: {
...
const ensureTopicOwnership = async (topicId, userId) => {
...
const ensurePageOwnership = async (pageId, userId) => {
...
const ensureKeywordOwnership = async (keywordId, userId) => {
...
const extractDomainKeywords = (domain) => domain?.keywords?.map((keyword) => keyword.term).filter(Boolean);
/**
 * GET /api/campaigns
 * Get all campaigns for user's company domain
 */
router.get('/', auth_1.authenticateToken, asyncHandler(async (req, res) => {
    const authReq = req;
 ...
 */
router.get('/:id/structure', auth_1.authenticateToken, asyncHandler(async (req, res) => {
    const authReq = req;
    const userId = authReq.user.userId;
 ...
 */
router.post('/:id/topics', auth_1.authenticateToken, asyncHandler(async (req, res) => {
    const authReq = req;
    const userId = authReq.user.userId;
    const campaignId = parseInt(req.params.id, 10);
    const { title, description } = req.body;
    ...
    await prisma.campaignTopic.create({
        data: {
            ...
            source: prisma_1.CampaignNodeSource.MANUAL
        }
    });
 ...
 */
router.post('/:id/topics/ai', auth_1.authenticateToken, asyncHandler(async (req, res) => {
    const authReq = req;
    const userId = authReq.user.userId;
    const campaignId = parseInt(req.params.id, 10);
    ...
    const generatedTopics = await (0, campaignAiService_1.generateCampaignTopics)({
        ...
        for (const generatedTopic of generatedTopics) {
            const topic = await tx.campaignTopic.create({
                data: {
                    ...
                    order: orderCursor++,
                    source: prisma_1.CampaignNodeSource.AI,
                    aiMetadata: {
            ...
            if (generatedTopic.pillarPage) {
                const pillar = await tx.campaignPage.create({
                    data: {
                        topicId: topic.id,
                        pageType: prisma_1.CampaignPageType.PILLAR,
            ...
            if (generatedTopic.subPages?.length) {
                ...
                for (const subPage of generatedTopic.subPages) {
                    const page = await tx.campaignPage.create({
                        data: {
                            topicId: topic.id,
                            pageType: prisma_1.CampaignPageType.SUBPAGE,
                            title: subPage.title,
 ...
 */
router.delete('/topics/:topicId', auth_1.authenticateToken, asyncHandler(async (req, res) => {
    const authReq = req;
    const userId = authReq.user.userId;
 ...
 */
router.post('/topics/:topicId/pillar', auth_1.authenticateToken, asyncHandler(async (req, res) => {
    const authReq = req;
    const userId = authReq.user.userId;
    const topicId = parseInt(req.params.topicId, 10);
    ...
    const existingPillar = await prisma.campaignPage.findFirst({
        where: { topicId, pageType: prisma_1.CampaignPageType.PILLAR }
    });
    if (existingPillar) {
 ...
 */
router.post('/topics/:topicId/pillar/ai', auth_1.authenticateToken, asyncHandler(async (req, res) => {
    const authReq = req;
    const userId = authReq.user.userId;
    const topicId = parseInt(req.params.topicId, 10);
    if (isNaN(topicId)) {
    ...
    const suggestion = await (0, campaignAiService_1.generatePillarPageSuggestion)({
        domainUrl: domain.url,
        domainContext: domain.context,
        keywords: extractDomainKeywords(domain),
 ...
 */
router.delete('/topics/:topicId/pillar', auth_1.authenticateToken, asyncHandler(async (req, res) => {
    const authReq = req;
    const userId = authReq.user.userId;
    const topicId = parseInt(req.params.topicId, 10);
    if (isNaN(topicId)) {
 ...
 */
router.post('/topics/:topicId/subpages', auth_1.authenticateToken, asyncHandler(async (req, res) => {
    const authReq = req;
    const userId = authReq.user.userId;
    const topicId = parseInt(req.params.topicId, 10);
    const { title, summary } = req.body || {};
    ...
    const maxOrder = await prisma.campaignPage.aggregate({
        where: { topicId, pageType: prisma_1.CampaignPageType.SUBPAGE },
        _max: { order: true }
    });
    await prisma.campaignPage.create({
        data: {
 ...
 */
router.post('/topics/:topicId/subpages/ai', auth_1.authenticateToken, asyncHandler(async (req, res) => {
    const authReq = req;
    const userId = authReq.user.userId;
    const topicId = parseInt(req.params.topicId, 10);
    ...
    const suggestions = await (0, campaignAiService_1.generateSubPagesSuggestion)({
        domainUrl: domain.url,
        domainContext: domain.context,
        keywords: extractDomainKeywords(domain),
        topicTitle: topic.title,
 ...
 */
router.delete('/pages/:pageId', auth_1.authenticateToken, asyncHandler(async (req, res) => {
    const authReq = req;
    const userId = authReq.user.userId;
 ...
 */
router.post('/pages/:pageId/keywords', auth_1.authenticateToken, asyncHandler(async (req, res) => {
    const authReq = req;
    const userId = authReq.user.userId;
    const pageId = parseInt(req.params.pageId, 10);
 ...
 */
router.post('/pages/:pageId/keywords/ai', auth_1.authenticateToken, asyncHandler(async (req, res) => {
    const authReq = req;
    const userId = authReq.user.userId;
    const pageId = parseInt(req.params.pageId, 10);
    ...
    const suggestions = await (0, campaignAiService_1.generateKeywordsSuggestion)({
        domainUrl: domain.url,
        domainContext: domain.context,
        keywords: extractDomainKeywords(domain),
        topicTitle: topic?.title,
 ...
 */
router.delete('/keywords/:keywordId', auth_1.authenticateToken, asyncHandler(async (req, res) => {
    const authReq = req;
    const userId = authReq.user.userId;
