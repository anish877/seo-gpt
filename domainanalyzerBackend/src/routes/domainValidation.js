...
router.post('/validate-domain', auth_1.authenticateToken, asyncHandler(async (req, res) => {
    try {
...
router.post('/check-ssl', auth_1.authenticateToken, asyncHandler(async (req, res) => {
    try {
        const { domain } = req.body;
        if (!domain) {
            return res.status(400).json({
...
router.post('/analyze-server', auth_1.authenticateToken, asyncHandler(async (req, res) => {
    try {
        const { domain } = req.body;
        if (!domain) {
            return res.status(400).json({
...
router.post('/configure-geo', auth_1.authenticateToken, asyncHandler(async (req, res) => {
    try {
...
async function analyzeServerResponse(url) {
    return new Promise((resolve) => {
        const timeout = 10000;
        const timer = setTimeout(() => {
...
async function generateLocationDomainContext(domain, location) {
    try {
